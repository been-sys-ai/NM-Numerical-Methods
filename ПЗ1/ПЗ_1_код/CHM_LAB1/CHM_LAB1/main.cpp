...
void printGrid(const vector<double> &x) {
    //Фиксируем вывод чисел с плавающей точкой с 6 занаками после запятой
    ...
    // В цикле выводим соответствующие значения x и h построчно
...
class CubicSpline {
    ...
    public:
        ...
        // Функция формирования кубического сплайна
        void build(const vector<double> &x_val, const vector<double> &y_val) {
            ...
            // Проверяем, что количество точек минимум две, чтобы был хотя бы один сегмент
            if (x_val.size() < 2) {
            ...
            // Выделяем память под соответствующие векторы
            a.resize(n + 1);
            b.resize(n + 1);
            c.resize(n + 1);
            d.resize(n + 1);
            ...
            if (n >= 2) {
                // Элементы первой строки не изменяются
                sigma[1] = beta[1];
                psi[1] = phi[1];
        ...
        double calculate_error(const double &a, const double &b, int derivative) const {
            // Проходимся по всем элементам массива точек и вычисляем значение функции в этих точках
...
int main() {
    ...
    // Осуществляем ввод границ отрезка и числа элементов разбиения
    cout << "Введите a и b: ";
    cin >> a >> b;
    ...
    // Вычисляем значения первой и второй производной в точках исходного равномерного разбиения при использовании сплайна
    for (int i = 0; i < x.size(); i++) {
    ...
    // В рамках вычисления дискретной производной функции методом конечных разностей будем рассматривать метод численного дифференцирования 2-го порядка точности.
    // Поскольку мы используем в расчетах равномерное разбиение, то формулы производных 1-го и 2-го порядка примут вид
